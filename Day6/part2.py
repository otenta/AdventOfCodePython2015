...
def create_brightness_grid():
    brightness_grid = [[0 for column in range(1000)] for row in range(1000)]
    return brightness_grid


def update_grid(passed_grid: list, new_coordinates: tuple[int, int], passed_brightness: list, state: State) -> tuple[list, list]:
    ...
    if state == state.ON:
        for row in range(initial_place[0], final_place[0] + 1):
            for column in range(initial_place[1], final_place[1] + 1):
                ...
                passed_brightness[row][column] +=1
    if state == state.OFF:
        for row in range(initial_place[0], final_place[0] + 1):
            for column in range(initial_place[1], final_place[1] + 1):
                if passed_brightness[row][column] == 0:
                    continue
                passed_brightness[row][column] -= 1
    if state == state.TOGGLE:
        for row in range(initial_place[0], final_place[0] + 1):
            for column in range(initial_place[1], final_place[1] + 1):
                    passed_brightness[row][column] += 2

    return passed_grid, passed_brightness
...
def find_brightness(final_brightness: list) -> int:
    brightness = 0
    for row in range (0, 1000):
        for column in range(0, 1000):
            brightness += final_brightness[row][column]
    return brightness

with open("input.txt") as file:
    initial_grid = create_initial_array()
    initial_brightness_grid = create_brightness_grid()
    for i in file.readlines():
        ...
        if match_turn_on:
            ...
            initial_grid, initial_brightness_grid = update_grid(initial_grid, coords_on, initial_brightness_grid, state=State.ON)
        if match_turn_off:
            coords_off = tuple(map(int, match_turn_off.groups()))
            initial_grid, initial_brightness_grid = update_grid(initial_grid, coords_off, initial_brightness_grid, state=State.OFF)
        if match_toggle:
            coords_toggle = tuple(map(int, match_toggle.groups()))
            initial_grid, initial_brightness_grid = update_grid(initial_grid, coords_toggle, initial_brightness_grid, state=State.TOGGLE)
    print(find_lights_on(initial_grid))
    print(find_brightness(initial_brightness_grid))
